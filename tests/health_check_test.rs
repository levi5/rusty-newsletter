...
#[tokio::test]
async fn subscribe_returns_a_status_200_for_valid_form_data() {
  let app = spawn_app().await;
  let client = reqwest::Client::new();

  // Act
  let body = "name=Jon%20Doe&email=jon_doe%40gmail.com";
  let response = client
  ...
  assert_eq!(saved.email, "jon_doe@gmail.com");
  assert_eq!(saved.name, "Jon Doe");
}

#[tokio::test]
async fn subscribe_returns_a_status_400_when_data_is_missing() {
  // Arrange
  let app = spawn_app().await;
  let client = reqwest::Client::new();
  let test_cases = vec![
    ("name=Jon%20Doe", "missing the email"),
    ("email=jon_doe%40gmail.com", "missing the name"),
